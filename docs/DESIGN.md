...
## Release-candidate density and selection policy

Predator Tactics deliberately ships as a large 25-ability library, which means raw candidate count can otherwise overwhelm smaller libraries even when individual base weights are sensible. The RC therefore uses soft scoring conflicts rather than hard caps. Every Predator Tactics entry carries three internal pool-density tags and conflicts with those tags. Once one Predator Tactics ability has been selected, later Predator candidates receive a substantial score penalty, allowing core abilities to compete again. Because this is a score adjustment instead of an exclusion, selecting Predator Tactics as the only ability source still fills the requested ability slots normally.

Passive and reaction abilities carry one additional type-density conflict. This discourages packages made almost entirely of passive modifiers or reactions while preserving thematic combinations when the candidate pool is narrow. Hard family exclusions remain reserved for genuinely redundant mechanics such as `pounce`, `predator-pursuit`, `predator-finisher`, and `predator-initiative`.
